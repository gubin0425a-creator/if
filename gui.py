...
class ChronosGUI:
    ...
    def setup_ui(self):
        ...
        self.duration_scale = tk.Scale(g1, from_=1, to=30, variable=self.duration_var, orient=tk.HORIZONTAL, bg="#181825", fg="#ffffff", troughcolor="#313244", activebackground="#89b4fa", highlightthickness=0, bd=0, font=("Malgun Gothic", 9))
    ...
    def start_video_revive(self):
        ...
        def worker():
            try:
                ...
                # 분석 후 추천 시간초로 슬라이더(duration_var) 자동 동기화
                rec_dur = report.get("recommended_duration")
                if rec_dur:
                    self.root.after(0, lambda d=rec_dur: self.duration_var.set(d))
                    self.log(f"⏱️  소생 분석 추천에 의해 장면 시간초가 {rec_dur}초로 자동 최적화되었습니다.\n")
